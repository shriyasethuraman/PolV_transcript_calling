...
setwd("/home/shriyas/cifs-lab/RIP_manuscript/Revised_Figures/Fig4/Fig4B/sense_temp_files/")
#Calculate scaled profile from CSV files
#If need to plot more than one profile, combine them by cbind
profile_scaled <- calculate_scaled_data("input1.csv","input2.csv", 39.33, 27.61)

#Plot the profile in the PDF format
pdf("col_profile_scaled.pdf", width=8, height=5)
...
reduced <- profile_scaled[seq(1, nrow(profile_scaled), 1),]						#keeps every N-th sequence	
png("col_scaled_heatmap.png", width=ncol(reduced), height=nrow(reduced), units="px")	
#par(mar=c(0,0,0,0))
...
profile_unscaled <- calculate_unscaled_data("H3K4me2_input.csv", "H3_input.csv", "H3_LENGTHS.csv", 2.31, 5.8)
